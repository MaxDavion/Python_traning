# -*- coding: utf-8 -*-
from model.group import Group

def test_create_group(app):
    old_groups = app.group.get_group_list()
    group = Group("groupname", "groupheader", "groupfooter")
    app.group.create(group)
    new_groups = app.group.get_group_list() ## Проверить, что после создания кол-во групп увеличилось на 1
    assert len(old_groups) + 1 == len(new_groups)
    old_groups.append(group)
    assert sorted(old_groups, key=Group.id_or_max) == sorted(new_groups, key=Group.id_or_max)

def test_create_empty_group(app):
    old_groups = app.group.get_group_list() #  список групп на странице до выполнения действия
    group = Group("", "", "")
    app.group.create(group)
    new_groups = app.group.get_group_list() # список групп на странице после выполнения действий
    assert len(old_groups) + 1 == len(new_groups)   ## Проверить, что после создания кол-во групп увеличилось на 1
    old_groups.append(group) # список групп - оракул
    assert sorted(old_groups, key=Group.id_or_max) == sorted(new_groups, key=Group.id_or_max)


